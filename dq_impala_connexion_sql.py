# -*- coding: utf-8 -*-
"""
Recette Python Dataiku — Data Quality sur une table Impala (v3)
----------------------------------------------------------------
Entrée  : une CONNEXION SQL Dataiku (pas de dataset d'entrée)
          + le nom de la table à analyser ("schema.table")
...
La recette n'a plus besoin de dataset d'entrée dans le Flow :
  - le schéma de la table est récupéré via DESCRIBE sur la connexion,
  - tous les contrôles SQL sont exécutés directement sur la connexion.
Le(s) dataset(s) de sortie restent à déclarer comme sorties de la recette.
...
# ==================================================================
SQL_CONNECTION = "test co"          # nom de la connexion SQL/Impala dans DSS
SOURCE_TABLE   = "default.tables"   # table à analyser, format "schema.table"
...
# ----- Typologie des colonnes (types NATIFS Impala/Hive via DESCRIBE) -----
COMPLEX_TYPES = {"array", "map", "struct"}          # non profilables en SQL
NUMERIC_TYPES = {"tinyint", "smallint", "int", "integer", "bigint",
...
def base_type(t):
    """Normalise un type SQL natif :
    decimal(10,2) -> decimal, varchar(50) -> varchar, array<string> -> array"""
    t = str(t).lower().strip()
    for sep in ("(", "<"):
        i = t.find(sep)
        if i >= 0:
...
# ==================================================================
# 2. CONNEXION SQL ET LOCALISATION DE LA TABLE
# ==================================================================
from dataiku import SQLExecutor2

executor = SQLExecutor2(connection=SQL_CONNECTION)
# Variante si nécessaire pour une connexion Impala "pure" :
#   from dataiku.core.sql import ImpalaExecutor
#   executor = ImpalaExecutor(connection=SQL_CONNECTION)
...
# ==================================================================
# 3. SCHÉMA DE LA TABLE VIA DESCRIBE (plus de dataset d'entrée)
# ==================================================================
desc = executor.query_to_df("DESCRIBE {}".format(full_table))
desc.columns = [str(c).lower().strip() for c in desc.columns]
name_col = "name" if "name" in desc.columns else desc.columns[0]
type_col = "type" if "type" in desc.columns else desc.columns[1]

schema_cols, _seen = [], set()
for _, r in desc.iterrows():
    n = str(r[name_col]).strip() if pd.notnull(r[name_col]) else ""
    t = str(r[type_col]).strip() if pd.notnull(r[type_col]) else ""
    # Ignore les lignes de séparation type Hive ("# Partition Information",
    # lignes vides) et les colonnes listées en double (partitions)
    if not n or n.startswith("#") or not t or n.lower() in _seen:
        continue
    _seen.add(n.lower())
    schema_cols.append({"name": n, "type": t})

if not schema_cols:
    raise ValueError(
        "Aucune colonne détectée via DESCRIBE sur {} (connexion '{}'). "
        "Vérifiez SOURCE_TABLE et SQL_CONNECTION.".format(
            table_label, SQL_CONNECTION))

print("Table {} : {} colonnes détectées via la connexion '{}'".format(
    table_label, len(schema_cols), SQL_CONNECTION))

# ==================================================================
# 4. REQUÊTE DE PROFILING (un seul passage sur la table)
# ==================================================================
simple_cols  = [c for c in schema_cols
                if base_type(c["type"]) not in COMPLEX_TYPES]
complex_cols = [c for c in schema_cols
                if base_type(c["type"]) in COMPLEX_TYPES]

distinct_expr = "COUNT(DISTINCT {q})" if EXACT_DISTINCT else "NDV({q})"

select_parts = ["COUNT(*) AS `__total_rows`"]
for c in simple_cols:
    n, t = c["name"], base_type(c["type"])
    q = "`{}`".format(n)
...
print("OK : {} colonnes analysées pour {} via la connexion '{}' (run du {})".format(
    len(df_out), table_label, SQL_CONNECTION,
