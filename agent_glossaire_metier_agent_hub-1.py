...
(version STREAMING + ARTEFACT compatible AGENT HUB)
=====================================================
Correctif Agent Hub (DSS 14.x) : l'onglet « Downloads » d'Agent Hub
n'affiche que deux formats d'artefacts :
  - artefacts de type RECORDS (tables -> .csv généré automatiquement),
  - artefacts contenant une part DATA_INLINE : {name avec extension,
    mimeType, dataBase64}.
Un artefact « TEXT » n'y est pas rendu. Le glossaire est donc émis ici
comme fichier .md encodé en base64 dans une part DATA_INLINE : il
apparaît en carte de téléchargement dans « See details > Downloads ».

Fonctionnement inchangé par ailleurs : streaming des étapes et des
appels d'outils SQL, glossaire rédigé token par token, export .md
optionnel dans un managed folder.
...
class MyLLM(BaseLLM):
    ...
    def process_stream(self, query, settings, trace):
        ...
        # ---- ARTEFACT au format « Downloads » d'Agent Hub -------------- #
        # Agent Hub affiche : artefacts de type RECORDS (-> .csv auto) ou
        # parts DATA_INLINE {name avec extension, mimeType, dataBase64}.
        fname = "glossaire_%s_%s.md" % (re.sub(r"\W+", "_", table),
                                        datetime.now().strftime("%Y%m%d-%H%M"))
        artifact = {
            "id": "glossaire-%s" % re.sub(r"\W+", "-", table),
            "name": fname,  # titre de la carte de téléchargement (garder l'extension)
            "description": "Glossaire métier généré le %s."
                           % datetime.now().strftime("%d/%m/%Y %H:%M"),
            "parts": [{
                "type": "DATA_INLINE",
                "mimeType": "text/markdown",
                "dataBase64": base64.b64encode(
                    glossary_md.encode("utf-8")).decode("ascii"),
            }],
        }
        yield {"chunk": {"artifacts": [artifact]}}
        yield self._txt("\n\n---\n📦 **%s** est disponible au téléchargement : "
                        "bouton *See details* sous la réponse, onglet *Downloads*."
                        % fname)
            ...
            with trace.subspan("Export vers managed folder"):
                try:
                    dataiku.Folder(EXPORT_FOLDER_ID).upload_data(
                        fname, glossary_md.encode("utf-8"))
                    yield self._txt("\n💾 Fichier `%s` également écrit dans le "
                                    "managed folder `%s`." % (fname, EXPORT_FOLDER_ID))
                except Exception as e:
